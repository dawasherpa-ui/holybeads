...
export default function Page({ params }: any) {
  ...
  React.useEffect(() => {
    const fetch = async () => {
      try {
        // const res = await AxiosInstance.get(`/subcategories/${subId}`);
  ...
  React.useEffect(() => {
    const fetch = async () => {
      try {
        const res = await AxiosInstance.get(`/products/?subcategory=${subcategory?._id}`);
    ...
    <div className="">
      <div className=" relative flex items-center justify-center ">
        ...
        <div className="space-y-3 text-center absolute px-4  z-10">
          <h1 className="text-3xl md:text-6xl text-white/80 font-semibold">{subcategory?.name}</h1>
        </div>
